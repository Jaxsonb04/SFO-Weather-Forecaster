...
# takes the engineered feature csv and produces plots + summary stats.
...
def get_features(use_cache=True):
    """load engineered features. builds from db if cache missing."""
...
def plot_patterns(df):
    ...
    # convert to pacific time so the diurnal cycle shows correctly
...
def plot_annual_overlay(df):
    ...
    # drop weeks with fewer than 3 days of data to avoid gaps creating flat lines
...
def print_summary(df):
    ...
    for target in ["target_temp_next_24h", "target_temp_next_48h"]:
        if target not in df.columns:
            continue
        top = (
            df.select_dtypes(include="number").corr()[target]
            .drop(["target_temp_next_24h", "target_temp_next_48h"], errors="ignore")
