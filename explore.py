...
if len(missing) == 0:
    ...
    out = pd.DataFrame({"missing": missing, "pct": pct})
    print(out.to_string())
...
print(f"expected: {span_hours:,} hourly rows (per station)")
print(f"actual:   {len(df):,} rows")

print("\nrows per year")
yearly = df.groupby(df["timestamp"].dt.year).size().rename("rows")
expected_per_year = {y: (366 if y % 4 == 0 and (y % 100 != 0 or y % 400 == 0) else 365) * 24
...
# detect gaps per station to avoid false gaps from interleaved timestamps
print("\nhourly gaps (>1h)")
...
if len(dupes) == 0:
    ...
    print(dupes.rename("duplicates").to_string())
