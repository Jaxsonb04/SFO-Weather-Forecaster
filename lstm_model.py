# lstm_model.py
# lstm forecaster for temperature prediction.
# complements xgboost -- different inductive bias, minimal feature engineering.
...
# raw meteorological signals only -- lstm creates its own lag features implicitly
LSTM_FEATURES = [
...
class SequenceDataset(Dataset):
    """yields (X[t-seq_len:t], y[t]) windows. skips any window containing NaN."""
    def __init__(self, features, targets, seq_len):
        self.features = features
        self.targets  = targets
        self.seq_len  = seq_len

        # vectorized validity check via cumulative nan count
        T = len(targets)
        ...
        t_idx = np.arange(seq_len, T)
        nans_in_window = cum[t_idx] - cum[t_idx - seq_len]
    ...
    def __getitem__(self, idx):
        ...
        X = self.features[t - self.seq_len:t]
        y = self.targets[t]
        return torch.FloatTensor(X), torch.FloatTensor([y])


class LSTMForecaster(nn.Module):
    """lstm -> final hidden state -> dense(h/2) -> dense(1)."""
...
def prepare_data(df, target, feature_cols, ratios=(0.7, 0.15, 0.15)):
    """chronological split matching xgboost. scaler fit on training data only."""
    ...
    X[:i1] = scaler.fit_transform(X[:i1])
    X[i1:] = scaler.transform(X[i1:])

    splits = {
        "train": (X[:i1],   y[:i1],   df.index[:i1]),
        "val":   (X[i1:i2], y[i1:i2], df.index[i1:i2]),
        "test":  (X[i2:],   y[i2:],   df.index[i2:]),
    }
