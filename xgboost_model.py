# model.py
# two xgboost forecasters:
#   1. target_temp_next_24h         -- spot temp 24h from now
#   2. target_daily_high_next_day   -- peak temp during the next calendar day
...
EXCLUDE = {
    # targets -- label leakage if kept
    "target_temp_next_24h", "target_temp_next_48h",
    "target_daily_high_next_day",

    # unit-converted duplicates (r = 1.0 with kept version)
    "temp_c", "dew_point_c", "wet_bulb_c", "wind_speed_ms",

    # redundant via eda correlation heatmap
    "dewpoint_depression",  # -0.98 corr with humidity

    # metadata
    "station_id", "station_name", "latitude", "longitude", "elevation",

    # raw cyclical components (sin/cos versions used instead)
    "wind_dir",

    # non-numeric or duplicated
...
def split_data(df, feature_cols, target, ratios=(0.7, 0.15, 0.15)):
    ...
    return {
        "train": (X.iloc[:i1],   y.iloc[:i1]),
        "val":   (X.iloc[i1:i2], y.iloc[i1:i2]),
        "test":  (X.iloc[i2:],   y.iloc[i2:]),
    }


def compute_sample_weights(y, alpha=2.0):
    """upweight extreme temps so the model takes heatwaves seriously."""
    z = (y - y.mean()) / y.std()
    return 1.0 + alpha * (z ** 2)


def tune_hyperparameters(X_train, y_train, weights_train):
    """grid search with time-series cv -- expanding window folds respect chronology."""
    grid = {
        "max_depth":        [6, 8],
        "learning_rate":    [0.05],
...
def train_xgboost(X_train, y_train, X_val, y_val, weights_train, params):
    """final fit with tuned params, sample weighting, and mae loss."""
...
def persistence_baseline(df, target):
    """predict current temp for spot target, today's daily high for daily-high target."""
...
def climatology_baseline(df_train, df_test, target):
    """predict the historical avg for this (day-of-year, hour) pair."""
    df_train = df_train.copy()
    df_train["doy"] = df_train.index.dayofyear
    df_train["hr"]  = df_train.index.hour
    climatology = df_train.groupby(["doy", "hr"])[target].mean()

    df_test = df_test.copy()
    df_test["doy"] = df_test.index.dayofyear
    df_test["hr"]  = df_test.index.hour
    return df_test.apply(
        lambda r: climatology.get((r["doy"], r["hr"]), np.nan),
        axis=1
    )
...
def stratified_eval(y_test, preds, target):
    ...
    def report(label, mask):
        if mask.sum() == 0:
            print(f"  {label:18s}  (no rows)"); return
        sub = err[mask]
...
def plot_error_breakdown(y_test, preds, target):
    err = pd.DataFrame({
        ...
        "hour":    y_test.index.tz_convert("America/Los_Angeles").hour,
